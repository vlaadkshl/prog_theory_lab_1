...
class SubjectControllerTest {
    ...
    void getAllSubjectsReturnsEmptyListWhenNoSubjectsExist() throws Exception {
        when(subjectServiceImpl.getAllSubjects()).thenReturn(Collections.emptyList());

        mockMvc.perform(get("/api/subject/all"))
                .andExpect(status().isOk())
                .andExpect(content().json("[]"));

        verify(subjectServiceImpl, times(1)).getAllSubjects();
    }

    @Test
    void getSubjectReturnsSubjectWhenExists() throws Exception {
        Subject subject = Subject.builder().id(1L).build();
        when(subjectServiceImpl.getSubject(1L)).thenReturn(subject);

        mockMvc.perform(get("/api/subject/1"))
                .andExpect(status().isOk())
                .andExpect(content().json("{\"id\":1}"));

        verify(subjectServiceImpl, times(1)).getSubject(1L);
    }

    @Test
    void getSubjectReturnsNotFoundWhenNotExists() throws Exception {
        when(subjectServiceImpl.getSubject(1L)).thenThrow(new ResourceNotFoundException("Subject not found"));
    ...
    void addSubjectReturnsCreatedSubject() throws Exception {
        Subject subject = Subject.builder().id(1L).build();
        when(subjectServiceImpl.addSubject(any(Subject.class))).thenReturn(subject);
        ...
        verify(subjectServiceImpl, times(1)).addSubject(any(Subject.class));
    }
}